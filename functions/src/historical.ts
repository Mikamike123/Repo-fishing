...
// --- 1. CONSTANTES PHYSIQUES v5.3.2 (STABILISATION THERMIQUE) ---
...
function solveAir2Water(history: any[], morphoId: MorphologyID, bassin: BassinType, depthId: DepthCategoryID, meanDepth?: number): number {
    ...
    // delta = inertie thermique (plus D est grand, plus c'est lent)
    const delta = morphoId === 'Z_RIVER' ? 12 : 0.207 * Math.pow(D, 1.35); 
    // mu = influence du rayonnement solaire (ajusté pour limiter la dérive)
    const mu = 0.15 + (1 / (D * 5));
    ...
    if (history && Array.isArray(history)) {
        history.forEach((day) => {
            ...
            // Calcul de l'oscillation solaire saisonnière
            const solarCorrection = mu * Math.sin((2 * Math.PI * (dayOfYear - PHI)) / 365);
            
            // Michael : Correction de la dérive. 
            // On calcule une température d'équilibre vers laquelle l'eau converge.
            const Tair = (day.temperature || 10) + offset;
            const equilibriumTemp = Tair + (solarCorrection * 10);
            
            // L'ajustement quotidien est une fraction de la distance vers l'équilibre
    ...
    // Michael : Sécurité physique pour la Seine (Bornes historiques 3°C - 26.5°C)
    return isNaN(waterTemp) ? 12 : Math.max(3, Math.min(26.5, waterTemp));
}
...
function calculateRD(lux: number, ntu: number, species: string): number {
    ...
    const p = params[species as keyof typeof params] || { rdMax: 3.0, kL: 0.20, kNTU: 0.05 };
...
export const getHistoricalContext = onCall({ region: "europe-west1" }, async (request) => {
    ...
    logger.info(`Simulation v5.3.2 corrigée Michael terminée. Tw: ${Tw.toFixed(1)}°C, Septembre maîtrisé.`);
    return response;
});