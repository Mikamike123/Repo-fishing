...
function solveAir2Water(history: any[], morphoId: MorphologyID, bassin: BassinType, depthId: DepthCategoryID, meanDepth?: number): number {
    ...
    return Math.max(3, Math.min(29.5, waterTemp));
}
...
function solveUltreiaFlow(history: any[]) {
    ...
    const trend: 'Montée' | 'Décrue' | 'Stable' = derivative > 0.5 ? 'Montée' : (derivative < -0.5 ? 'Décrue' : 'Stable');
...
function calculateRD(lux: number, ntu: number, species: string): number {
    const params: any = {
        ...
        'Black-Bass': { rdMax: 4.5, kL: 0.20, kNTU: 0.04 } 
    };
...
function getSeasonalMetabolicPotential(date: Date, species: string): number {
    ...
    if (species === 'Black-Bass') {
        if (month >= 11 || month <= 2) return 0.20; 
        if (month >= 5 && month <= 8) return 1.0;   
        return 0.60; 
    }
    if (month >= 11 || month <= 2) return species === 'Brochet' ? 0.85 : 0.70; 
    if (month >= 5 && month <= 8) return 1.0; 
    return 0.90; 
}
...
const computeUltreiaScore = (species: string, ctx: BioContext, lux: number, oxyF: number, crep: number): number => {
    ...
    const visualFactor = 1 + (1.5 * (rd / (2.0 + rd)));
    ...
    if (ctx.waterTemp < tRef && species !== 'Brochet' && species !== 'Black-Bass') tSigma = tSigma * 1.5; 
    ...
    if (species === 'Brochet') {
        ...
        if (ctx.pressureTrend > 5 && ctx.cloudCover < 20) si_baro = 0.4;
        else if (Math.abs(ctx.pressureTrend) < 2) si_baro = 1.1;
        else si_baro = 0.9;
    } else {
    ...
    if (finalScore > 80) finalScore = 80 + (20 * (1 - Math.exp(-(finalScore - 80) / 15)));
...
export const getHistoricalContext = onCall({ region: "europe-west1" }, async (request) => {
    // 1. VALIDATION Michael : Sécurisation du payload d'entrée
    const validation = HistoricalInputSchema.safeParse(request.data);
    if (!validation.success) {
        throw new HttpsError("invalid-argument", "Données de contexte historique incomplètes.");
    }
    ...
    if (startTime && endTime) {
        ...
        } catch (e) { logger.warn("Error calculating median time", e); }
    }
    ...
    const meanDepth = morpho?.meanDepth;
    ...
    // [FIX] Michael : Casting explicite vers DepthCategoryID pour satisfaire le compilateur
    const Tw = solveAir2Water(
        historyToUse, 
        morphoId, 
        bassin, 
        (morpho?.depthId || 'Z_3_15') as DepthCategoryID, 
        meanDepth
    );
    ...
    const context: BioContext = {
        ...
        waveHeight: wavesHs, flowIndex: intensity, flowDerivative: derivative, 
        flowTrend: trend, date: sessionDate, morphoId: morphoId
    };
    ...
    const response: FullEnvironmentalSnapshot = {
        ...
        hydro: {
            ...
            dissolvedOxygen: DO, waveHeight: wavesHs, flowRaw: Math.round(intensity) 
        },
        scores: {
