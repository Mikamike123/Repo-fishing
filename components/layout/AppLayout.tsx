    ...
    return (
        <div className={`min-h-screen transition-colors duration-700 ${isActuallyNight ? 'bg-[#1c1917] text-stone-300' : 'bg-[#FAF9F6] text-stone-600'} pb-32 relative`}>
            {userProfile?.pendingLevelUp && (
                <LevelUpModal level={userProfile.levelReached || 1} onClose={() => engine.handleConsumeLevelUp(false)} onConfirm={() => engine.handleConsumeLevelUp(true)} />
            )}
...
const NavBtn = ({ engine, view, icon, label, color }: any) => (
    <button onClick={() => { engine.triggerHaptic([5]); engine.setCurrentView(view); }} className={`flex flex-col items-center gap-1.5 transition-all active:scale-90 ${engine.currentView === view ? color : 'text-stone-500 font-black'}`}>
    ...
    <>
        ...
        <aside className={`fixed top-0 left-0 h-full w-4/5 max-w-xs z-[60] shadow-2xl p-8 animate-in slide-in-from-left flex flex-col pt-[env(safe-area-inset-top,24px)] ${engine.isActuallyNight ? 'bg-[#1c1917] text-stone-200 border-r border-stone-800' : 'bg-white text-stone-600'}`}>
            ...
            <nav className="space-y-3 flex-1">
                ...
                <button onClick={() => { engine.setCurrentView('profile'); engine.setIsMenuOpen(false); }} className={`w-full flex items-center justify-between p-5 rounded-2xl transition-all font-black text-xl group ${engine.isActuallyNight ? 'hover:bg-stone-800 text-stone-300' : 'hover:bg-amber-50 text-stone-600'}`}><span className="flex items-center gap-4"><User size={28} className="text-stone-400 group-hover:text-amber-500"/> Ton Profil</span><ChevronRight size={24} /></button>
            </nav>
...
const LevelUpModal = ({ level, onClose, onConfirm }: any) => (
