...
export const AppLayout = ({ engine, children }: { engine: any, children: React.ReactNode }) => {
    const { 
        isActuallyNight, triggerHaptic, setIsMenuOpen, userProfile, isOnline, 
        handleMagicDiscovery, arsenalData, currentUserId, isMenuOpen, 
        currentView, setCurrentView, hasNewFeed // Michael : On anticipe la pastille
    } = engine;
        ...
        <div className={`min-h-screen transition-colors duration-700 ${isActuallyNight ? 'bg-[#1c1917] text-stone-300' : 'bg-[#FAF9F6] text-stone-600'} pb-32 relative`}>
            ...
            <header className={`sticky top-0 z-30 ${isActuallyNight ? 'bg-[#292524]/90 border-stone-800' : 'bg-white/90 border-stone-200'} backdrop-blur-lg border-b px-5 pt-[calc(env(safe-area-inset-top)+14px)] pb-5 flex items-center justify-between shadow-sm`}>
                <div className="flex items-center gap-3">
                    <button onClick={() => { triggerHaptic([10]); setIsMenuOpen(true); }} className={`p-2 transition-all relative oracle-btn-press ${isActuallyNight ? 'text-stone-400 hover:text-amber-400' : 'text-stone-500 hover:text-stone-800'}`}>
                        <Menu size={28} strokeWidth={2.5} />
                        {/* Pastille sur le burger si nouveauté dans une vue du menu (ex: Secteurs) */}
                        {engine.hasNewMenuContent && (
                            <span className="absolute top-1 right-1 w-2.5 h-2.5 bg-amber-500 border-2 border-[#FAF9F6] dark:border-[#292524] rounded-full" />
                        )}
                    </button>
            ...
            <nav className={`fixed bottom-0 left-0 right-0 z-40 border-t ${isActuallyNight ? 'bg-[#292524]/95 border-stone-800' : 'bg-white/95 border-stone-200'} backdrop-blur-md pb-[env(safe-area-inset-bottom,12px)] shadow-lg`}> 
                <div className="mx-auto grid grid-cols-6 max-w-lg items-center py-4 px-1">
                    ...
                    <NavBtn engine={engine} view="feed" icon={<Zap size={26}/>} label="FEED" color="text-indigo-600" hasBadge={hasNewFeed} />
...
const NavBtn = ({ engine, view, icon, label, color, hasBadge }: any) => (
    <button onClick={() => { engine.triggerHaptic([5]); engine.setCurrentView(view); }} className={`relative flex flex-col items-center gap-1.5 transition-all oracle-btn-press ${engine.currentView === view ? color : 'text-stone-500 font-black'}`}>
        {icon}
        <span className="text-[10px] uppercase tracking-tighter font-black">{label}</span>
        {hasBadge && (
            <span className="absolute top-0 right-1/4 w-2.5 h-2.5 bg-amber-500 border-2 border-white dark:border-[#292524] rounded-full animate-pulse" />
        )}
    </button>
);
...
const MenuEntry = ({ icon, label, onClick, isActuallyNight }: any) => (
    <button onClick={onClick} className={`w-full flex items-center justify-between p-5 rounded-2xl transition-all font-black text-xl group oracle-btn-press ${isActuallyNight ? 'hover:bg-stone-800 text-stone-300' : 'hover:bg-amber-50 text-stone-600'}`}>
