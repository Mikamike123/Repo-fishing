...
import { 
    ...
    Thermometer, AlertCircle, Sun, CloudRain, Cloud, MapPin
} from 'lucide-react'; 
...
const DataInfoPopup: React.FC<{ message: string; show: boolean; onClose: () => void }> = ({ message, show, onClose }) => (
    show ? (
        // Changement de w-64 à w-80 et ajout de whitespace-normal
...
const Dashboard: React.FC<DashboardProps> = ({ sessions, onDeleteSession }) => {
    ...
    useEffect(() => {
        const loadRealtimeData = async () => {
            ...
            // L'appel retourne maintenant 'hydroMessage'
            ...
            setHydroInfoMessage(hydroMessage); // Capture le message Hydro
            ...
            setWaterTempInfoMessage(tempMsg); // Capture le message Température
            setIsLoading(false);
        };
        loadRealtimeData();
    }, []);
    ...
    // NOTE: Simuler les deltas passés pour la pression et le débit pour le BioScore, 
    // L'idéal serait de stocker l'historique en Firestore, mais ici on simule le delta pour le calcul.
    const deltaP = 1.0; 
    const deltaQ = 5.0;
    
    const bioConditions: BioConditions = useMemo(() => {
        // Construction de la structure BioConditions requise par calculateBioScore
        // Les valeurs nulles sont sécurisées à 0 pour le calcul
        return {
            date: new Date(),
            currentWeather: realtimeWeather || { temperature: 0, pressure: 0, clouds: 0, windSpeed: 0 },
            currentHydro: realtimeHydro || { flow: 0, level: 0 },
            // Estimation des données passées pour le calcul de la variation
    ...
    // Derived Values for UI (basés sur bioConditions)
    ...
    // Get recent sessions (last 2) (Inchangé)
    ...
    // Rendu pour l'icône de météo Air
    ...
    const renderWaterTempWidget = () => {
        ...
        return (
            <div className={`rounded-2xl p-3 flex flex-col items-center text-center border ${tempBg} h-[72px] justify-center relative`}>
                {isLoading ? (
                ...
                <div className="relative z-10">
                    <div className="flex justify-between items-start mb-6">
                        <div className="flex items-center gap-2">
                            ...
                            <h3 className="text-stone-800 font-bold text-lg">État du Spot (Nanterre / Austerlitz)</h3>
                        </div>
                    ...
                    {/* Sub-Indicators - VIBRANT UPDATE (Grille de 5 Colonnes) */}
                    <div className="grid grid-cols-5 gap-3">
                        
                        {/* 1. Température Air (Nanterre) */}
                        <div className="bg-red-50 rounded-2xl p-3 flex flex-col items-center text-center border border-red-100 h-[72px] justify-center">
                        ...
                        {/* 2. Pression Atmosphérique (Nanterre) */}
                        <div className="bg-indigo-50 rounded-2xl p-3 flex flex-col items-center text-center border border-indigo-100 h-[72px] justify-center">
                        ...
                        {/* 3. Débit (Austerlitz) */}
                        <div className="bg-cyan-500/5 rounded-2xl p-3 flex flex-col items-center text-center border border-cyan-100 h-[72px] justify-center relative">
                            <Droplets size={16} className="text-cyan-500 mb-2" />
                            <span className="text-xs font-bold text-cyan-900">
                                {isLoading ? '...' : realtimeHydro?.flow?.toFixed(2) ?? 'N/A'}
                            </span>
                        ...
                        {/* 4. Niveau / Hauteur d'eau (Austerlitz) */}
                        <div className="bg-blue-50 rounded-2xl p-3 flex flex-col items-center text-center border border-blue-100 h-[72px] justify-center relative">
                        ...
                        {/* 5. Température de l'Eau (Austerlitz - J-1) */}
                        {renderWaterTempWidget()}
                    </div>
                    {/* --- FIN GRILLE 5 COLONNES --- */}
