// components/OracleHero.tsx - Version 12.0.0 (New Species Palette & Confidence)
// Michael : Harmonisation de la couleur Perche (Violet Améthyste) et injection confiance.

import React, { useState, useEffect, useMemo } from 'react';
import { ChevronDown, Zap, Calendar, MapPin, BarChart2, Target, ShieldCheck } from 'lucide-react';
...
const OracleHero: React.FC<OracleHeroProps> = ({ 
    ...
    const calculateConfidence = (timestamp: number): number => {
        ...
        const trendConfidence = 74 - ((diffHours - 120) * 0.15);
        return Math.max(40, Math.round(trendConfidence));
    };
            ...
            if (dataPoints && dataPoints.length > 0) {
                return dataPoints.map(pt => {
                    const filteredPoint: any = {
                        ...
                        confidence: calculateConfidence(pt.timestamp)
                    };
                    allowedKeys.forEach(key => {
                        if ((pt as any)[key] !== undefined) filteredPoint[key] = (pt as any)[key];
                    });
    ...
    const chartSubTitle = mode === 'single'
        ? 'Simulation Bio-Physique (14 Jours)'
        : `Analyse comparative sur ${favorites.length} secteurs`;
    ...
    const summaryInfo = useMemo(() => {
        ...
        return {
            ...
            confidence: calculateConfidence(current.timestamp)
        };
                    ...
                    <div>
                        <div className="flex items-center gap-2">
                            <h2 className={`text-lg font-black leading-none tracking-tight ${titleColor}`}>
                                {mode === 'compare' ? 'COMPARATIF' : 'ORACLE PRO'}
                            </h2>
                            ...
                            <div className={`flex items-center gap-1 mt-1 text-sm font-medium ${subTitleColor}`}>
                                {mode === 'single' ? (
                                    <>
                                        ...
                                        {summaryInfo && (
                                            <span className={`flex items-center gap-1 font-bold ml-1 px-1.5 rounded text-xs ${
                                                ...
                                                <ShieldCheck size={10} className={summaryInfo.confidence > 80 ? "text-emerald-500" : "text-amber-500"} />
                                            </span>
                                        )}
                                    </>
                                ) : (
