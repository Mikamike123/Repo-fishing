// components/SessionFormUI.tsx - Version 4.9.5 (Tactile Form Upgrade)
...
const SessionFormUI: React.FC<SessionFormUIProps> = (props) => {
    ...
    return (
        <div className={`rounded-3xl p-6 shadow-2xl pb-24 animate-in fade-in slide-in-from-bottom-4 duration-500 transition-all ${
            ...
            <div className="flex justify-between items-center mb-6">
                ...
                {/* Michael : Bouton X Tactile */}
                <button type="button" onClick={() => onCancel ? onCancel() : window.location.reload()} className={`p-2.5 rounded-full transition-all oracle-btn-press ${isActuallyNight ? 'bg-stone-800 text-stone-400' : 'bg-stone-100 text-stone-500'}`}>
            ...
            <form onSubmit={handleFormSubmit} className="space-y-4">
                ...
                <div className="grid grid-cols-12 gap-2">
                    ...
                    <div className="col-span-4">
                        ...
                        <input type="time" required step="60" value={startTime} onChange={e => setStartTime(e.target.value)} className={`w-full p-3 rounded-xl font-bold text-xs outline-none border transition-all ${inputBg}`} />
                    </div>
                    <div className="col-span-4">
                        <label className="text-[9px] font-bold text-stone-400 uppercase tracking-widest mb-1 block">Fin (24h)</label>
                        <input type="time" required step="60" value={endTime} onChange={e => setEndTime(e.target.value)} className={`w-full p-3 rounded-xl font-bold text-xs outline-none border transition-all ${inputBg}`} />
                    </div>
                </div>

                {/* SECTEUR / SPOT */}
                <div className="grid grid-cols-2 gap-2">
                    <div className="transition-all oracle-btn-press">
                ...
                {/* COMBO */}
                <div className="grid grid-cols-1 gap-2 transition-all oracle-btn-press">
                ...
                {/* TOGGLE CAPOT SLIM - Michael : Devient tactile */}
                <div 
                    onClick={() => setIsCapotExpress(!isCapotExpress)}
                    className={`flex items-center justify-between px-3 py-2 rounded-2xl border cursor-pointer transition-all oracle-btn-press ${
                    ...
                    <div className={`w-10 h-5 rounded-full relative transition-colors ${isCapotExpress ? 'bg-amber-500' : (isActuallyNight ? 'bg-stone-700' : 'bg-stone-300')}`}>
                ...
                {!isCapotExpress && (
                    <div className="space-y-4 pt-1 animate-in fade-in duration-300">
                        <div className="flex gap-3">
                            <button type="button" onClick={() => setIsMissModalOpen(true)} className={`flex-1 py-3.5 rounded-2xl text-[11px] font-black border shadow-sm active:scale-95 transition-all oracle-btn-press flex justify-center items-center gap-2 ${
                            ...
                            <button type="button" onClick={() => setIsCatchModalOpen(true)} className={`flex-1 py-3.5 rounded-2xl text-[11px] font-black border shadow-sm active:scale-95 transition-all oracle-btn-press flex justify-center items-center gap-2 ${
                        ...
                        {/* LISTES DES PRISES / RATÉS Tactiles */}
                        <div className="space-y-2">
                            {catches.map(c => (
                                <div key={c.id} className={`flex items-center justify-between p-3 border rounded-xl transition-all oracle-card-press ${isActuallyNight ? 'bg-emerald-900/10 border-emerald-900/30' : 'bg-emerald-50/50 border-emerald-100'}`}>
                                    ...
                                    <div className="flex gap-1 shrink-0">
                                        <button type="button" onClick={() => { setEditingCatch(c); setIsCatchModalOpen(true); }} className={`p-2 rounded-full shadow-sm border transition-all oracle-btn-press ${isActuallyNight ? 'bg-stone-800 border-stone-700 text-stone-400' : 'bg-white border-stone-100 text-stone-400'}`}><Copy size={12}/></button>
                                        <button type="button" onClick={() => triggerDelete(c.id, 'catch')} className={`p-2 rounded-full shadow-sm border transition-all oracle-btn-press ${isActuallyNight ? 'bg-stone-800 border-stone-700 text-rose-500' : 'bg-white border-stone-100 text-rose-400'}`}><X size={12}/></button>
                                    </div>
                                </div>
                            ))}
                            {misses.map(m => (
                                <div key={m.id} className={`flex items-center justify-between p-3 border rounded-xl transition-all oracle-card-press ${isActuallyNight ? 'bg-rose-900/10 border-rose-900/30' : 'bg-rose-50/50 border-rose-100'}`}>
                                    <span className={`font-bold text-sm flex items-center gap-2 truncate ${isActuallyNight ? 'text-stone-200' : 'text-stone-800'}`}>
                                        <div className="w-2 h-2 rounded-full bg-rose-500"></div>
                                        {m.type}
                                    </span>
                                    <div className="flex gap-1 shrink-0">
                                        <button type="button" onClick={() => { setEditingMiss(m); setIsMissModalOpen(true); }} className={`p-2 rounded-full shadow-sm border transition-all oracle-btn-press ${isActuallyNight ? 'bg-stone-800 border-stone-700 text-stone-400' : 'bg-white border-stone-100 text-stone-400'}`}><Copy size={12}/></button>
                                        <button type="button" onClick={() => triggerDelete(m.id, 'miss')} className={`p-2 rounded-full shadow-sm border transition-all oracle-btn-press ${isActuallyNight ? 'bg-stone-800 border-stone-700 text-rose-500' : 'bg-white border-stone-100 text-rose-400'}`}><X size={12}/></button>
                                    </div>
                                </div>
                            ))}
                        </div>
                ...
                <div className="pt-2">
                      ...
                      <input type="range" min="1" max="10" value={feelingScore} onChange={e => setFeelingScore(parseInt(e.target.value))} className={`w-full h-2 rounded-lg appearance-none cursor-pointer accent-amber-500 transition-all oracle-btn-press ${isActuallyNight ? 'bg-stone-800' : 'bg-stone-200/50'}`}/>
                </div>
                ...
                {/* VALIDATION FINALE TACTILE */}
                <button type="submit" disabled={isLoadingEnv} className={`w-full py-5 rounded-2xl font-black shadow-xl active:scale-95 transition-all oracle-btn-press flex items-center justify-center gap-2 text-lg uppercase tracking-tighter ${
