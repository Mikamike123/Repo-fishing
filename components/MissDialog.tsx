// components/MissDialog.tsx - Version 10.0.3 (Anti-Bubbling Shield Edition)
...
const MissDialog: React.FC<MissDialogProps> = ({ 
  ...
  const modalContent = (
    ...
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-200 p-4 h-[100dvh] w-full overflow-hidden">
      ...
      {/* Michael : Ajout de stopPropagation pour empêcher les clics de remonter au SessionFormUI parent */}
      <div 
        onClick={(e) => e.stopPropagation()}
        className={`relative w-full max-w-lg h-fit max-h-[90dvh] rounded-3xl shadow-2xl p-6 space-y-5 border overflow-y-auto transition-colors duration-500 ${
        ...
        {/* Michael : On stoppe aussi la propagation du submit pour éviter de déclencher le formulaire de session en cascade */}
